# Citations

This repository is an original *Vitis*-oriented **functional annotation playbook** (plus optional upstream structural notes).  
Cite the **tools and papers** you actually run; do not treat this file as a claim of authorship transfer.

## Functional annotation (main product)

| Resource | Citation / venue |
|----------|------------------|
| eggNOG-mapper v2 | Cantalapiedra et al. *MBE* 2021 doi:10.1093/molbev/msab293 |
| eggNOG databases | Huerta-Cepas et al.; Hernández-Plaza et al. *NAR* (resource issues) |
| InterPro / InterProScan | Blum / Paysan-Lafosse et al. *NAR* InterPro updates |
| DIAMOND | Buchfink et al. |
| AHRD | Schoof lab AHRD; see tool docs |
| MapMan4 / Mercator4 | Schwacke et al. *Molecular Plant* 2019 doi:10.1016/j.molp.2019.01.003 |
| iTAK | Zheng et al. *Molecular Plant* 2016 doi:10.1016/j.molp.2016.09.014 |
| OrthoFinder | Emms & Kelly |
| Trinotate | Bryant et al. / Trinotate project |
| HRP (NB-LRR) | Andolfo et al. / HRP software |

Journal-tier METHODS notes: [`RECENT_HIGH_QUALITY.md`](RECENT_HIGH_QUALITY.md).

## Upstream structural (optional)

| Resource | Citation / venue |
|----------|------------------|
| BRAKER3 | Gabriel et al. *Genome Research* 2024 doi:10.1101/gr.278090.123 |
| BRAKER / TSEBRA / GALBA | Hoff, Brůna, Gabriel, Stanke et al. |
| EvidenceModeler (EVM) | Haas et al. |
| PASA | Haas et al. |
| GSAman | Chen, Chen & Xia *The Innovation* 2026 doi:10.1016/j.xinn.2026.101537 |
| SynGAP | Wu et al. *Genome Biology* 2024 |
| Liftoff / LiftOn | Shumate & Salzberg; further LiftOn literature |
| EDTA | Ou et al. |
| BUSCO / compleasm / OMArk / PSAURON / AGAT | Respective papers / software notes |

## Related software (further reading)

Catalog of commonly used annotation tools and workflows (not vendored here):  
[`RELATED_SOFTWARE.md`](RELATED_SOFTWARE.md).

## Structural / AI ab initio (upstream; cite if used)

| Tool | Citation cue |
|------|----------------|
| Helixer | Holst et al. *Nat Methods* 2025 doi:10.1038/s41592-025-02939-1 |
| Tiberius | Gabriel et al. *Bioinformatics* 2024 doi:10.1093/bioinformatics/btae685 |
| ANNEVO | Zhang, Ye et al. (Kai Ye lab); https://github.com/xjtu-omics/ANNEVO — confirm venue when citing |
| MAKER2 | Holt & Yandell *BMC Bioinformatics* 2011 doi:10.1186/1471-2105-12-491 |
| AUGUSTUS / GeneMark / EVM / PASA | Cite the versions actually run |

## FA KO / GO extras (cite if used)

| Tool | Citation cue |
|------|----------------|
| BlastKOALA / GhostKOALA | Kanehisa et al. *JMB* 2016 |
| DeepKOALA | Briefings in Bioinformatics / GenomeNet — confirm on use |
| DeepGOPlus | Kulmanov et al. lineage — confirm version/paper on use |
| EviAnn (structure upstream) | *Nat Methods* 2026 doi:10.1038/s41592-026-03156-0 |
| Earl Grey (masking) | Baril et al. *MBE* 2024 doi:10.1093/molbev/msae068 |
| MetaEuk | Levy Karin et al. *Microbiome* 2020 |

## Deep hunt extras (cite if used)

| Tool | Citation cue |
|------|----------------|
| IsoQuant | Prjibelski et al. *Nat Biotechnol* 2023 doi:10.1038/s41587-022-01565-y |
| SQANTI3 | Pardo-Palacios et al. *Nat Methods* 2024 doi:10.1038/s41592-024-02229-2 |
| OMArk | Nevers et al. *Nat Biotechnol* 2024 doi:10.1038/s41587-024-02147-w |
| compleasm | Huang et al. (miniprot+BUSCO completeness) |
| ProteInfer | Google Research — confirm paper on use |
| EGAPx / Gnomon | NCBI CGR / RefSeq EGAP docs |
